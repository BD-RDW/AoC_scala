...
object day01_2 {
  ...
  def findFrequence(str: String): Int = {
    var foundFrequency: Int = -1
    var frequencies: Array[Int] = Array(0)
    var total = 0
    while (foundFrequency == -1) {
      str.trim().split(",").foreach(s => {
        if (s.trim().charAt(0) == '+') total += s.trim().substring(1).toInt
        else total -= s.trim().substring(1).toInt
        if (frequencies.find(total == _) != None && (foundFrequency == -1)) foundFrequency = total
  ...
  def main(args: Array[String]): Unit = {
    println(f"Result of input ${input.substring(0, 20.min(input.length))}%s is ${findFrequence(input)}%d")
  }
}
