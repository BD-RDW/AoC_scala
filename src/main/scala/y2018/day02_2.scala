...
object day02_2 {
  ...
  def main(args: Array[String]): Unit = {
    println(f"From he input of ${input.length}%d ID's 2 Id's heb ben found with these common characters ${findCommonCharacters(input)}%s")
  }
  def findCommonCharacters(strArr: Array[String]): String = {
    var foundIds = findTheTwoIdsThatOnlyDifferOneCharacter(strArr)
    foundIds(0).iterator.zip(foundIds(1).iterator).filter(p => p._1 == p._2).map{_._1}.mkString
  }
  def findTheTwoIdsThatOnlyDifferOneCharacter(strArr: Array[String]): Array[String] = {
    var foundStrings: Array[String] = Array()
    for(i <- 0 to strArr.length - 2) {
      for(j <- i + 1 to strArr.length - 1) {
        if (foundStrings.isEmpty && strArr(i).iterator.zip(strArr(j).iterator).filter(p => p._1 != p._2).length == 1)
          foundStrings = Array(strArr(i), strArr(j))
      }
    }
    foundStrings
  }
