...
object day02_2 {
  ...
  def main(args: Array[String]): Unit = {
    println(f"In the input of ${input.length}%d passwords there are ${checkPasswords(input)}%d password valid.")
  }

  def extractDataFromLine(str: String): (Int, Int, String, String) = {
    val parser: Regex = "(\\d+)-(\\d+) (\\S*): (\\S+)".r
    str match {
      case parser(pos1, pos2, chr, pw) => (pos1.toInt, pos2.toInt, chr, pw)
    }
  }

  def isPasswordValid(str: String): Boolean = {
    val(pos1,pos2,chr,pw) = extractDataFromLine(str)
    (pw.charAt(pos1-1) == chr.charAt(0) || pw.charAt(pos2-1) == chr.charAt(0)) && (! (pw.charAt(pos1-1) == chr.charAt(0) && pw.charAt(pos2-1) == chr.charAt(0)))
  }

  def checkPasswords(strArr: Array[String]): Int = {
    strArr.filter(pw => isPasswordValid(pw)).length
  }
