...
object day02_1 {
  ...
  def extractDataFromLine(str: String): (Int, Int, String, String) = {
    ...
    str match {
      case parser(min, max, chr, pw) => (min.toInt, max.toInt, chr, pw)
    }
  }

  def chrCount(pw: String, chr: String) = {
    pw.toCharArray.filter(c => c == chr.charAt(0)).length
  }

  def isPasswordValid(str: String): Boolean = {
    val(min,max,chr,pw) = extractDataFromLine(str)
    val cnt = chrCount(pw, chr)
    cnt >= min && cnt <= max
  }
