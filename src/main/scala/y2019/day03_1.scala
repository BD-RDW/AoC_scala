...
object day03_1 {
  ...
  def determinelineIntersections(lines: Array[Array[(String, Int,Int)]]): List[(Int, Int)] = {
    var intersections: List[(Int,Int)] = List()
    for(i <- 1 to lines(0).length - 1) {
      ...
      for(j <- 1 to lines(1).length - 1) {
        if (lines(0)(i)._1 != lines(1)(j)._1) {
          ...
          if (intersection != null) intersections = intersections :+ intersection
        }
      }
    }
    intersections.distinct.filter(i => i != (0,0))
  }
  ...
  def calculateLines(movement: Array[Array[(String, Int)]]): Array[Array[(String, Int,Int)]] = {
    var coordinate = (0,0)
    val temp1:Array[(String, Int,Int)] = movement(0).map(m => {
      coordinate = (coordinate._1 + getHorizontalMove(m._1, m._2), coordinate._2 + getVerticalMove(m._1, m._2))
      (getOrientation(m._1), coordinate._1, coordinate._2)
    })
    coordinate = (0,0)
    val temp2:Array[(String, Int,Int)] = movement(1).map(m => {
      coordinate = (coordinate._1 + getHorizontalMove(m._1, m._2), coordinate._2 + getVerticalMove(m._1, m._2))
      (getOrientation(m._1), coordinate._1, coordinate._2)
    })
    Array(
  ...
  def determineClosestIntersection(intersections: List[(Int, Int)]):(Int,Int) = {
    var point = intersections(0)
    var dist = point._1.abs + point._2.abs
    for(i <- 1 to intersections.length - 1) {
      val point2 = intersections(i)
      val dist2 = point2._1.abs + point2._2.abs
      if (dist > dist2) {
        dist = dist2
        point = point2
      }
  ...
  def calculate(strArr: Array[String]): Int = {
    ...
    val lines: Array[Array[(String, Int,Int)]] = calculateLines(movement)
    val intersections:List[(Int,Int)] = determinelineIntersections(lines)
    val closestIntersection = determineClosestIntersection(intersections)
    closestIntersection._1.abs + closestIntersection._2.abs
  }
}
