...
object day03_2 {
  ...
  def main(args: Array[String]): Unit = {
    println(f"The distance to the closest intersection is ${calculate(input)}%d")
  }

  def processinput(strArr: Array[String]): Array[Array[(String, Int)]] = {
    Array(
      strArr(0).split(",").map(s => (s.substring(0,1), s.substring(1).toInt)),
      strArr(1).split(",").map(s => (s.substring(0,1), s.substring(1).toInt))
    )
  }

  def getIntersection(line1: ((Int, Int),(Int, Int)), line2: ((Int, Int),(Int, Int))): (Int,Int) = {
    var horLine = line1
    var vertLine = line2
    if (vertLine._1._1 != vertLine._2._1) {
      horLine = line2
      vertLine = line1
    }
    if ( (horLine._1._1.min(horLine._2._1) <= vertLine._1._1 && horLine._1._1.max(horLine._2._1) >= vertLine._1._1) &&
      (vertLine._1._2.min(vertLine._2._2) <= horLine._1._2 && vertLine._1._2.max(vertLine._2._2) >= horLine._1._2) )
      (vertLine._1._1, horLine._1._2)
    else
      null
  }

  def determinelineIntersections(lines: Array[Array[(String, Int,Int,Int)]]): List[(Int,Int,Int)] = {
    var intersections: List[(Int,Int,Int)] = List()
    for(i <- 1 to lines(0).length - 1) {
      val line1 = ((lines(0)(i-1)._2,lines(0)(i-1)._3),(lines(0)(i)._2, lines(0)(i)._3))
      for(j <- 1 to lines(1).length - 1) {
        if (lines(0)(i)._1 != lines(1)(j)._1) {
          val intersection = getIntersection(line1, ((lines(1)(j-1)._2,lines(1)(j-1)._3),(lines(1)(j)._2, lines(1)(j)._3)))
          if (intersection != null) {
            var distance = lines(0)(i-1)._4 + lines(1)(j-1)._4
            distance += (if (lines(0)(i-1)._1 == "V") (lines(0)(i-1)._2 - intersection._1).abs else (lines(0)(i-1)._3 - intersection._2).abs)
            distance += (if (lines(1)(j-1)._1 == "V") (lines(1)(j-1)._2 - intersection._1).abs else (lines(1)(j-1)._3 - intersection._2).abs)
            intersections = intersections :+ (intersection._1, intersection._2, distance)
          }
        }
      }
    }
    intersections.distinct.filter(i => i != (0,0,0))
  }

  def getHorizontalMove(dir: String, steps: Int): Int = {
    dir match {
      case "R" => steps
      case "L" => -steps
      case "U" => 0
  ...
  def getVerticalMove(dir: String, steps: Int): Int = {
    dir match {
      case "R" => 0
      case "L" => 0
      case "U" => steps
  ...
  def getOrientation(dir: String): String = {
    dir match {
      case "R" => "H"
      case "L" => "H"
      case "U" => "V"
  ...
  def calculateLines(movement: Array[Array[(String, Int)]]): Array[Array[(String, Int,Int,Int)]] = {
    var coordinate = (0,0)
    var distance = 0
    val temp1:Array[(String, Int,Int,Int)] = movement(0).map(m => {
      distance += m._2
      coordinate = (coordinate._1 + getHorizontalMove(m._1, m._2), coordinate._2 + getVerticalMove(m._1, m._2))
      (getOrientation(m._1), coordinate._1, coordinate._2, distance)
    })
    coordinate = (0,0)
    distance = 0
    val temp2:Array[(String, Int,Int,Int)] = movement(1).map(m => {
      distance += m._2
      coordinate = (coordinate._1 + getHorizontalMove(m._1, m._2), coordinate._2 + getVerticalMove(m._1, m._2))
      (getOrientation(m._1), coordinate._1, coordinate._2, distance)
    })
  ...
  def determineClosestIntersection(intersections: List[(Int, Int,Int)]):(Int,Int,Int) = {
    var point = intersections(0)
    for(i <- 1 to intersections.length - 1) {
      val point2 = intersections(i)
      if (point._3 > point2._3) {
        point = point2
      }
    }
    point
  ...
  def calculate(strArr: Array[String]): Int = {
    val movement: Array[Array[(String,Int)]] = processinput(strArr)
    val lines: Array[Array[(String, Int,Int,Int)]] = calculateLines(movement)
    val intersections:List[(Int,Int,Int)] = determinelineIntersections(lines)
    val closestIntersection = determineClosestIntersection(intersections)
    closestIntersection._3
  }
}
